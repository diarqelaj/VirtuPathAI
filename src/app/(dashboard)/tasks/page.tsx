  ...
  return (
    <div className="relative bg-black-100 text-white min-h-screen flex flex-col">
     <main className="flex-1 pt-12 px-4 sm:px-6 md:px-12 max-w-3xl mx-auto w-full">
      <div className="bg-black-100/80 border border-white/10 backdrop-blur-xl p-6 sm:p-8 md:p-10 rounded-3xl shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="text-center mb-10 sm:mb-12">
            <SparklesIcon className="h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 text-purple-400/30 mx-auto mb-2 sm:mb-4" />
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-purple-300 to-blue-300 leading-snug">
            ...
            {loading ? (
              ...
              {tasks.map((task, index) => (
                <div
                  key={task.id}
                  className={`flex items-start sm:items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl border ${
                ...
                >
                  {task.checked ? (
                    <CheckCircleIcon className="h-5 w-5 sm:h-6 sm:w-6 text-gray-400 mt-1 sm:mt-0.5" />
                  ) : (
                    <svg className="h-5 w-5 sm:h-6 sm:w-6 text-gray-400 mt-1 sm:mt-0.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                      <circle cx="12" cy="12" r="10" />
                    </svg>
                  )}
                  <span className={`text-base sm:text-lg font-medium tracking-wide ${task.checked ? 'text-gray-200' : 'text-gray-400'}`}>
                    {task.text}
                  </span>
                </div>
              ))}
