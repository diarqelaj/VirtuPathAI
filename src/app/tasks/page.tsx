...
const TaskPage = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        // Select the most recent subscription
        const activeSubscription = subscriptions.reduce((latest, current) => {
          // Compare subscription start dates and return the one that is most recent
        ...
        console.log('Using Career Path ID:', careerPathID);

        // Use lastAccessedDay or default to 1 if not available
        let startDay = activeSubscription.lastAccessedDay || 1;
        console.log('Using start day:', startDay);

        // Fetch tasks based on the most recent career path ID and the user's start day
        const tasksRes = await api.get(`/DailyTasks/bycareerandday?careerPathId=${careerPathID}&day=${startDay}`);
        console.log('API Response:', tasksRes.data);
  ...
  // Function to send task completion data to the server
  const recordTaskCompletion = async (taskID: number) => {
    ...
    const completionData = {
      completionID: 0, // Assuming this will be auto-generated by the backend
    ...
    try {
      await api.post('/TaskCompletion', completionData); // Assuming the endpoint for task completion is '/TaskCompletion'
  ...
  const toggleTask = async (index: number) => {
    ...
    // Fetch the original DailyTask
    const orig = await api.get(`/DailyTasks/${t.id}`);
    const fullTask = orig.data;
    // apply whatever updates you need; for example, mark a boolean column if you have one
    fullTask.isCompleted = t.checked;  // only if your model has such a column
