...
export default function PricingPage() {
  ...
  return (
    <div className="relative bg-black-100 text-white min-h-screen flex flex-col overflow-hidden">
      ...
      <main className="relative z-10 pt-40 pb-24 px-6 md:px-20 flex-1">
            ...
            <>
              ...
              <div className="hidden md:grid grid-cols-3 gap-10">
                {["monthly", "onetime", "bonus"].map((plan) => {
                  ...
                  const paragraph =
                    plan === "monthly"
                      ? "Great for consistent learners who want affordable, daily progress with accountability."
                      : plan === "onetime"
                      ? "Unlock lifetime access to your career path, tasks, and insights with no subscription needed."
                      : "Get exclusive 1-on-1 AI mentorship, advanced analytics, and career certification.";
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-2xl border border-white/10 bg-white/5 p-6 min-h-[500px] transition-all cursor-pointer ${
                        isActive
                          ? "shadow-[0_0_40px_2px_rgba(162,100,255,0.25)] scale-[1.07]"
                          : "hover:shadow-[0_0_40px_2px_rgba(162,100,255,0.25)]"
                      }`}
                    >
                      <h3 className="text-xl font-semibold mb-1 [text-shadow:0_0_10px_rgba(162,100,255,0.6)]">{title}</h3>
                      <p className="text-sm text-white/70 mb-3">{paragraph}</p>
                      <div className="border-t border-dashed border-white/20 mb-4" />
                      <p className="text-[2rem] font-bold mb-4">${price}</p>
                      <button className={`w-full py-3 mb-5 rounded-lg font-semibold transition ${
                        plan === "onetime"
              ...
              <div className="md:hidden flex overflow-x-auto space-x-4 pt-12 px-1 pb-4 no-scrollbar">
                {["monthly", "onetime", "bonus"].map((plan, index) => {
                  const isActive = selectedPlan === plan;
                  const price =
                    plan === "monthly"
                      ? Math.ceil(selectedCareer.price / 12)
                  ...
                  const paragraph =
                    plan === "monthly"
                      ? "Perfect for steady learners who want consistent guidance and motivation."
                      : plan === "onetime"
                      ? "Ideal for those ready to commit to their full 365-day journey at once."
                      : "Best for growth-driven learners seeking full support, mentorship, and recognition.";
                  ...
                  return (
                    <div
                      key={plan}
                      onClick={() => setSelectedPlan(plan as any)}
                      className={`max-w-[250px] h-[410px] rounded-2xl border border-white/10 bg-white/5 p-6 transition-all flex-shrink-0 ${
                        isActive
                          ? "shadow-[0_0_40px_2px_rgba(162,100,255,0.25)] scale-[1.03]"
                          : "hover:shadow-[0_0_40px_2px_rgba(162,100,255,0.25)]"
                      }`}
                    >
                      <h3 className="text-lg font-semibold mb-1 [text-shadow:0_0_10px_rgba(162,100,255,0.6)]">
                        {plan === "bonus" ? (
                          <>
                      ...
                      <p className="text-sm text-white/70 mb-3">{paragraph}</p>
                      <div className="border-t border-dashed border-white/20 mb-4" />
                      <p className="text-[1.8rem] font-bold mb-4 [text-shadow:0_0_12px_rgba(162,100,255,0.6)]">
                        ${price}
                        {plan === "monthly" && <span className="text-base text-white/60"> /month</span>}
                      </p>
                      <button
                        className={`w-full py-2 mb-4 rounded-lg font-semibold transition text-sm ${
                          plan === "onetime"
                            ? "bg-[#8a4dff] hover:bg-[#5f4f7e] text-white"
                            : plan === "bonus"
                            ? "bg-yellow-400 hover:bg-yellow-300 text-black"
